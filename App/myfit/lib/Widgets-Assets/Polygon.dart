...
class HexagonFilled extends StatelessWidget
{
  Color color;
  Size size;
  double width;
  Widget _child;
  bool animated;
  final List<PolygonBoxShadow> shadows;

  HexagonFilled( this._child,{this.shadows:const [],this.animated=false,this.color:HColors.midnightTextPrimary, this.size:const Size(100.0,100.0), this.width=4.0});
  static int sides=6;
  ...
  Widget build(BuildContext context) {
    ...
    animated?new AnimateHexagonOutline(specs,[new PolygonLine(color:color)],size:size,width:width,color: color,capType:StrokeCap.square,style:PaintingStyle.fill):
    new PolygonOutline(specs,[new PolygonLine(color:color)],size:size,width:width,color: color,capType:StrokeCap.square,style:PaintingStyle.fill),
